...
def process(path): # processing fucntion
    ...
    for column in dataset.columns:
        for start in tqdm(range(0, len(new_data[column]), batch_size)):
            ...
            embeddings = max_pooling(output, encoded_data['attention_mask']).cpu().numpy()

            emb_dict[column].extend(embeddings.tolist())

    labels_list = []
    for col in columns:
        labels = KMeans(random_state=42, n_clusters=30).fit(np.array(emb_dict[col])).labels_
    ...
    for idx, col in enumerate(columns):
        ...
        for cluster in top_clusters:
            ...
            final_dict[array[indx][0]] = round(len(clusters_sentences[cluster]) / overall_len * 100, 1)
